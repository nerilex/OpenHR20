...
typedef struct
{
    uint8_t buffer_size; /**< holds the amount of bytes which may be stored in
                            the buffer */
    uint8_t fillcount;   /**< holds the amount of bytes actually stored in the
                            buffer */
 ...
 * \param cb buffer context to be initialized
 * \param buffer buffer for the storage of data (you are responisble for
 *allocation and freeing)
 */
void circularbytebuffer_init2(uint8_t buffersize, circularbytebuffer_t *cb,
                              void *buffer);
#endif
/** \brief
 *
...
uint16_t circularbytebuffer_peek_fifo(circularbytebuffer_t *cb);

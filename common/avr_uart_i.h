/* uart_i.h */
/*
    Copyright (C) 2006-2015 bg nerilex (bg@nerilex.org)
 ...
 *                      be send when crossed and an XOFF has been send
 *previously
...
typedef struct
{
    ...
    volatile uint8_t
        hook_running; /**< flag indicating if the hook is running */
#endif
#if UART0_SWFLOWCTRL
    volatile uint8_t
        txon; /**< flag indicating if we are allowed to send data */
 ...
 * \return zero if no data is available else a value different from zero is
 *returned
 */
 ...
 * When the hook is set to \a NULL (which is the default) incomming data is
 buffered
 * in a special ringbuffer and read by \ref uart0_getc(). If the hook is set to
 a
 * different value, this value is interpret as a function pointer. The hook (the
 * function where the function pointer points to) is called with the recieved
 data
 ...
 *
 * This function initializes the second uart according to the parameter
 *specifyed
 * in config.h .
 */
 ...
 * When the hook is set to \a NULL (which is the default) incomming data is
 buffered
 * in a special ringbuffer and read by \ref uart1_getc(). If the hook is set to
 a
 * different value, this value is interpret as a function pointer. The hook (the
 * function where the function pointer points to) is called with the recieved
 data
